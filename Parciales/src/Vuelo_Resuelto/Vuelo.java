...
public class Vuelo {
	...
	private Asiento buscarAsientoReservado(String fila, int numero) {
		...
		while (asientoABuscar == null && i < this.asientosReservados.size()) {
			if (this.asientosReservados.get(i).getFila().equals(fila) && this.asientosReservados.get(i).getNumero()==numero) {
	...
	public ResultadoReservaBoletosEnum reservarAsiento(int dni, String telefono, ClaseEnum clase, String fila, int numero) {

		ResultadoReservaBoletosEnum resultado = ResultadoReservaBoletosEnum.ERROR_AVION_COMPLETO;

		if (this.asientosReservados.size() < this.cantidadAsientos) {	// esto no podría pasar
			if (buscarAsientoReservado(fila,numero) == null) {	// si no está vendido
